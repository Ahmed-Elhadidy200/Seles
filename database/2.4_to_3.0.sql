...
ALTER TABLE `Elnokrashyospos_sales_suspended_items_taxes`
 MODIFY COLUMN `percent` decimal(15,3) NOT NULL;
...
ALTER TABLE `Elnokrashyospos_app_config`
 MODIFY COLUMN `key` varchar(50) NOT NULL,
 MODIFY COLUMN `value` varchar(500) NOT NULL;

UPDATE `Elnokrashyospos_app_config` SET `key` = 'receipt_show_total_discount' WHERE `key` = 'show_total_discount';

DELETE FROM `Elnokrashyospos_app_config` WHERE `key` = 'use_invoice_template';
DELETE FROM `Elnokrashyospos_app_config` WHERE `key` = 'language';
DELETE FROM `Elnokrashyospos_app_config` WHERE `key` = 'thousands_separator';
...
UPDATE `Elnokrashyospos_modules` SET `sort` = 110 WHERE `name_lang_key` = 'module_config';
...
UPDATE `Elnokrashyospos_employees` SET `hash_version` = 1;
