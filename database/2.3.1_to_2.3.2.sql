...
INSERT IGNORE INTO `Elnokrashyospos_permissions` (permission_id, module_id, location_id) 
(SELECT CONCAT('sales_', location_name), 'sales', location_id FROM Elnokrashyospos_stock_locations);

INSERT IGNORE INTO `Elnokrashyospos_permissions` (permission_id, module_id, location_id)
(SELECT CONCAT('receivings_', location_name), 'receivings', location_id FROM Elnokrashyospos_stock_locations);
